    ...
    const int **rowOffset, const int **colIndices, double **matrixValues, double **rhsVector, double *pressureOld = nullptr)
{
    ...
    if(idx < n){
        ...
        for (int face = 0; face < 4; ++face) {
            ...
            if (boundaryID == 0 || boundaryID == 2 || boundaryID == 6)
                continue;
...
class nacaAirfoilIntegrator : public NumericalIntegrator3D
{
public:
    nacaAirfoilIntegrator(const Mesh3D& mesh_)
...
void nacaAirfoilIntegrator::setupVelocityPrediction(std::array<SparseMatrixCSR, 3>& csrMatrix, std::array<deviceVector<double>, 3>& rhsVector, const std::array<deviceVector<double>, 3>& velocity)
{
    double* vel[3];
...
void nacaAirfoilIntegrator::setupPressure(SparseMatrixCSR& csrMatrix, deviceVector<double>& rhsVector, deviceVector<double>& solution, deviceVector<double>& solutionOld)
{
...
void nacaAirfoilIntegrator::setupVelocityCorrection(std::array<SparseMatrixCSR, 3>& csrMatrix, std::array<deviceVector<double>, 3>& rhsVector,
    const std::array<deviceVector<double>, 3>& velocity, const std::array<deviceVector<double>, 3>& velocityOld)
{
    double* vel[3];
    double* velOld[3];
...
void nacaAirfoilIntegrator::assembleVelocityPrediction()
{
    unsigned int blocks = blocksForSize(mesh.getCells().size);
...
void nacaAirfoilIntegrator::assemblePressureEquation()
{
    unsigned int blocks = blocksForSize(mesh.getCells().size);
...
void nacaAirfoilIntegrator::assembleVelocityCorrection()
{
    unsigned int blocks = blocksForSize(mesh.getCells().size);
...
int main(int argc, char *argv[]){
    ...
    Mesh3D mesh;
    if(!mesh.loadMeshFromFile("../airfoilMesh505K.dat"))
        return EXIT_FAILURE;
    ...
    hostParams.dt = 0.001;
    hostParams.mu = 0.001;
    hostParams.maxIterations = 10000;
    ...
    nacaAirfoilIntegrator integrator(mesh);
